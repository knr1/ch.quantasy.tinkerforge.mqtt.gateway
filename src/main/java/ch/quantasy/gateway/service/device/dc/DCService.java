...
public class DCService extends AbstractDeviceService<DCDevice, DCServiceContract> implements DCDeviceCallback {
    ...
    public void messageArrived(String string, MqttMessage mm) throws Exception {
        ...
        if (string.startsWith(getServiceContract().INTENT_TOPIC_ENABLED)) {
    ...
    public void PWMFrequencyChanged(Integer pwmFrequency) {
        addStatus(getServiceContract().STATUS_TOPIC_PWM_FREQUENCY, pwmFrequency);
    }

    @Override
    public void accelerationChanged(Integer acceleration) {
        addStatus(getServiceContract().STATUS_TOPIC_ACCELERATION, acceleration);
    }

    @Override
    public void driveModeChanged(Short driverMode) {
        addStatus(getServiceContract().STATUS_TOPIC_DRIVER_MODE, driverMode);
    }

    @Override
    public void minimumVoltageChanged(Integer minimumVoltage) {
        addStatus(getServiceContract().STATUS_TOPIC_MINIMUM_VOLTAGE, minimumVoltage);

    }

    @Override
    public void velocityPeriodChanged(Integer velocityPeriod) {
        addStatus(getServiceContract().STATUS_TOPIC_VELOCITY_PERIOD, velocityPeriod);
    }

    @Override
    public void velocityChanged(Short velocity) {
        addStatus(getServiceContract().STATUS_TOPIC_VELOCITY, velocity);
    }

    @Override
    public void currentVelocity(short s) {
        addEvent(getServiceContract().EVENT_TOPIC_VELOCITY, new Velocity(s));
    }

    @Override
    public void emergencyShutdown() {
        addEvent(getServiceContract().STATUS_TOPIC_EMERGENCY_SHUTDOWN, System.currentTimeMillis());
    }

    @Override
    public void underVoltage(int i) {
        addEvent(getServiceContract().EVENT_TOPIC_UNDERVOLTAGE, new UnderVoltage(i));
    }

    @Override
    public void velocityReached(short s) {
        addEvent(getServiceContract().EVENT_TOPIC_FULL_BRAKE, new Velocity(s));
    }

    @Override
    public void enabledChanged(Boolean isEnabled) {
        addStatus(getServiceContract().STATUS_TOPIC_ENABLED, isEnabled);
    }

    @Override
    public void fullBrake() {
        addEvent(getServiceContract().EVENT_TOPIC_FULL_BRAKE, System.currentTimeMillis());
    }

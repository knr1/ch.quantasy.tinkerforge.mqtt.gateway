 ...
 *  *
 */
package ch.quantasy.gateway.service.tinkerforge.stepper;
...
import ch.quantasy.gateway.message.stepper.DriveModeStatus;
import ch.quantasy.gateway.message.stepper.EnableStatus;
import ch.quantasy.gateway.message.stepper.MinimumVoltageStatus;
import ch.quantasy.gateway.message.stepper.RectificationStatus;
import ch.quantasy.gateway.message.stepper.StepMode;
import ch.quantasy.gateway.message.stepper.StepsStatus;
import ch.quantasy.gateway.message.stepper.TargetPositionStatus;
import ch.quantasy.gateway.message.stepper.CurrentPositionStatus;
import ch.quantasy.gateway.message.stepper.DecayStatus;
import ch.quantasy.gateway.message.stepper.MotorCurrentStatus;
import ch.quantasy.gateway.message.stepper.SpeedRampStatus;
import ch.quantasy.gateway.message.stepper.StepModeStatus;
import ch.quantasy.gateway.message.stepper.TimeBaseStatus;
import ch.quantasy.gateway.message.stepper.VelocityStatus;
...
public class StepperService extends AbstractDeviceService<StepperDevice, StepperServiceContract> implements StepperDeviceCallback {
    ...
    public void allDataPeriodChanged(Long allDataPeriod) {
        readyToPublishStatus(getContract().STATUS_ALL_DATA_PERIOD, new AllDataCallbackPeriodStatus(allDataPeriod));
    }

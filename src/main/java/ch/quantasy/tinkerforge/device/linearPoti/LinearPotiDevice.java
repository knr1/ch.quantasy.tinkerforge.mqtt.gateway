...
import ch.quantasy.gateway.binding.tinkerforge.linearPoti.LinearPotiIntent;
 ...
 */
public class LinearPotiDevice extends GenericDevice<BrickletLinearPoti, LinearPotiDeviceCallback, LinearPotiIntent> {

    public LinearPotiDevice(TinkerforgeStack stack, BrickletLinearPoti device) throws NotConnectedException, TimeoutException {
        super(stack, device, new LinearPotiIntent());
    }
    ...
    @Override
    public void update(LinearPotiIntent intent) {
        if (intent == null) {
            return;
        }
        if (!intent.isValid()) {
        ...
        if (intent.debouncePeriod != null) {
            try {
                ...
                Logger.getLogger(LinearPotiDevice.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
